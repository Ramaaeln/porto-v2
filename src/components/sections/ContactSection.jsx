...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="relative w-full min-h-screen bg-[#fafafa] dark:bg-[#0c0c0e] pt-20 md:pt-24 px-4 sm:px-6 py-12 md:py-16 transition-colors duration-500 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] sm:w-[600px] h-[300px] sm:w-[600px] bg-indigo-500/5 dark:bg-indigo-500/[0.02] blur-[80px] sm:blur-[130px] rounded-full pointer-events-none select-none" />
      ...
      <div className="max-w-4xl mx-auto z-10 relative space-y-6 sm:space-y-10">
        ...
        {/* Responsive Grid System untuk Tombol Social Media */}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-zinc-900 border-2 sm:border-4 border-zinc-950 dark:border-zinc-800 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.05)]">
          ...
          {/* Dinamis Height & Padding Penyesuaian Mobile */}
            ...
            {loading ? (
              ...
              <AnimatePresence initial={false}>
                {messages.map((m) => {
                  ...
                  return (
                    <motion.div key={m.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                      <div className={`flex gap-2 sm:gap-3 max-w-[90%] sm:max-w-[80%] ${mine ? "flex-row-reverse" : ""}`}>
                        ...
                        <div className="flex flex-col gap-1">
                          <div className={`flex items-center gap-1.5 select-none ${mine ? "flex-row-reverse" : ""}`}>
                            <span className="font-mono font-black text-[9px] sm:text-[10px] tracking-wide text-zinc-600 dark:text-zinc-400 truncate max-w-[80px] sm:max-w-none">{m.user_name}</span>
                            <span className={`px-1 py-0.5 text-[6px] sm:text-[7px] font-black font-mono border-[1px] sm:border-2 border-zinc-950 ${dev ? "bg-emerald-500 text-zinc-950 shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]" : "bg-indigo-600 text-white border-indigo-700"}`}>
                              {dev ? "CORE_DEV" : "GUEST"}
                            </span>
                          </div>
                          
                          <div className={`p-2 sm:p-3 border-2 text-[11px] sm:text-xs font-mono break-words shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-none ${mine ? "bg-zinc-950 text-emerald-400 border-zinc-950 dark:bg-indigo-600 dark:text-white dark:border-indigo-700" : "bg-white dark:bg-zinc-800 border-zinc-950 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100"}`}>
                            {m.content}
                          </div>
                        </div>
                      </div>
          ...
          <div className="p-3 sm:p-4 border-t-2 sm:border-t-4 border-zinc-950 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900/50">
            {user ? (

...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="relative w-full min-h-screen bg-[#fafafa] dark:bg-[#0c0c0e] pt-20 md:pt-24 px-4 sm:px-6 py-12 md:py-16 transition-colors duration-500 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] sm:w-[600px] h-[300px] bg-indigo-500/5 dark:bg-indigo-500/[0.02] blur-[80px] sm:blur-[130px] rounded-full pointer-events-none select-none" />
      ...
      <div className="max-w-4xl mx-auto z-10 relative space-y-6 sm:space-y-10">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-zinc-900 border-2 sm:border-4 border-zinc-950 dark:border-zinc-800 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.05)] flex flex-col">
          ...
          <div className="bg-zinc-950 dark:bg-zinc-800 px-3 sm:px-5 py-3 flex items-center justify-between border-b-2 sm:border-b-4 border-zinc-950 select-none">
          ...
          <div className="bg-zinc-100 dark:bg-zinc-950/60 border-b-2 border-zinc-950 dark:border-zinc-800 px-3 sm:px-5 py-2 flex items-center justify-between text-[8px] sm:text-[9px] font-mono font-bold text-zinc-500 dark:text-zinc-400 select-none gap-2">
          ...
          <div className="h-[320px] sm:h-[420px] overflow-y-auto bg-zinc-50/30 dark:bg-black/20 p-3 sm:p-6 space-y-4 sm:space-y-6 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700">
            {loading ? (
                ...
                {messages.map((m) => {
                  ...
                  return (
                    <motion.div key={m.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${mine ? "justify-end" : "justify-start"} group`}>
                      <div className={`flex gap-2 sm:gap-3 max-w-[95%] sm:max-w-[85%] items-end ${mine ? "flex-row-reverse" : ""}`}>
                        
                        <img src={m.user_avatar || `https://ui-avatars.com/api/?name=${m.user_name}&background=6366f1&color=fff`} className="w-7 h-7 sm:w-9 sm:h-9 border-[1px] sm:border-2 border-zinc-950 dark:border-zinc-700 rounded-none object-cover shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] dark:shadow-none shrink-0" alt="avatar" />
                        
                        <div className="flex flex-col gap-1 w-full">
                          <div className={`flex items-center gap-2 select-none ${mine ? "flex-row-reverse" : ""}`}>
                            <span className="font-mono font-black text-[9px] sm:text-[10px] tracking-wide text-zinc-600 dark:text-zinc-400 truncate max-w-[100px] sm:max-w-none">{m.user_name}</span>
                            <span className={`px-1.5 py-0.5 text-[6px] sm:text-[7px] font-black font-mono border-[1px] sm:border-2 border-zinc-950 ${dev ? "bg-emerald-500 text-zinc-950 shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]" : "bg-indigo-600 text-white border-indigo-700"}`}>
                              {dev ? "CORE_DEV" : "GUEST"}
                            </span>
                            
                            {/* Menampilkan format Tanggal & Jam di samping kanan lencana user */}
                            <span className="font-mono text-[8px] text-zinc-400 dark:text-zinc-500 tracking-tighter shrink-0">
                          ...
                          <div className={`p-2 sm:p-3 border-2 text-[11px] sm:text-xs font-mono shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-none ${mine ? "bg-zinc-950 text-emerald-400 border-zinc-950 dark:bg-indigo-600 dark:text-white dark:border-indigo-700" : "bg-white dark:bg-zinc-800 border-zinc-950 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100"}`}>
                            {m.reply_to && (
                              <div className={`mb-2 p-2 border-l-4 text-[9px] sm:text-[10px] opacity-80 ${mine ? "border-emerald-500/50 bg-white/5" : "border-zinc-400 bg-zinc-100 dark:bg-zinc-950 dark:border-zinc-600"}`}>
                                <div className="font-bold truncate">@{m.reply_to.user_name}</div>
                                <div className="truncate">{m.reply_to.content}</div>
                              </div>
                            )}
                        ...
                        {user && (
                          <button onClick={() => setReplyingTo(m)} className={`opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity p-1.5 text-zinc-400 hover:text-indigo-500 dark:hover:text-indigo-400 active:scale-95 shrink-0 ${mine ? "mr-1" : "ml-1"}`}>
          ...
          <div className="flex flex-col border-t-2 sm:border-t-4 border-zinc-950 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900/50">
            ...
            <AnimatePresence>
              {replyingTo && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-zinc-200 dark:bg-zinc-800 px-3 sm:px-4 py-2 border-b-2 border-zinc-950 dark:border-zinc-700 flex justify-between items-center overflow-hidden">
                  <div className="flex flex-col gap-0.5 truncate pr-2 border-l-4 border-indigo-500 pl-2">
            ...
            <div className="p-3 sm:p-4">
              {user ? (
                <div className="flex gap-2">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.96 }} onClick={sendMessage} className="bg-indigo-600 text-white px-4 sm:px-6 font-mono font-black text-[11px] sm:text-xs hover:bg-indigo-700 transition-all flex items-center gap-1.5 border-2 border-zinc-950 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none">
